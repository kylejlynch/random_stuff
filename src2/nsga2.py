...
class FitnessContext:
    ...
    def _evaluate_flagged_frame(
        self, flagged: SparkDataFrame, coverage: float
    ) -> FitnessResult:
        agg = (
            flagged.select(
    ...
    def evaluate(self, portfolio: RuleSet) -> FitnessResult:
        ...
        return self._evaluate_flagged_frame(flagged, coverage)

    def evaluate_flag_column(self, column: str) -> FitnessResult:
        if column not in self.frame.columns:
            raise ValueError(f"Column '{column}' not available in evaluation frame")

        flagged = self.frame.where(F.col(column) == 1)
        positives = flagged.count()
        coverage = float(positives) / self.total_records if self.total_records else 0.0
...
class PortfolioFactory:
    def __init__(
        ...
        self.min_conditions = max(1, min(min_conditions, self.max_conditions))
    ...
    def random_rule(self) -> Rule:
        ...
        upper = min(feature_count, self.max_conditions)
        lower = min(self.min_conditions, upper)
        lower = max(1, lower)
        if lower > upper:
            lower = upper

        num_conditions = upper if lower == upper else random.randint(lower, upper)
    ...
    def _normalise(self, rules: List[Rule]) -> List[Rule]:
        ...
        seen: set[str] = set()
        for rule in rules:
            if len(rule.conditions) < self.min_conditions:
                continue
    ...
    def normalise_portfolio(self, portfolio: RuleSet) -> RuleSet:
        rules = [rule.copy() for rule in portfolio.rules]
        return RuleSet([rule.copy() for rule in self._normalise(rules)])
    ...
    def mutate_rule(self, rule: Rule, mutation_rate: float = 0.2) -> Rule:
        ...
        if random.random() < 0.1 and len(mutated.conditions) > self.min_conditions:
...
class NSGAII:
    def __init__(
        ...
        max_rules: int = 4,
        random_seed: int = 42,
        seed_portfolios: Sequence[RuleSet] | None = None,
        min_conditions: int = 1,
    ) -> None:
        ...
        self.seed_portfolios = [
            portfolio.copy() for portfolio in (seed_portfolios or [])
        ]
    ...
    def _initial_population(self) -> List[Individual]:
        ...
        for seed in self.seed_portfolios:
            normalised = self.factory.normalise_portfolio(seed)
            population.append(self._evaluate_portfolio(normalised))
