...
def load_training_view(
    ...
    required_columns = [LABEL_COLUMN, AMOUNT_COLUMN] + list(NUMERIC_FEATURES)
    missing_required = [col for col in required_columns if col not in base_frame.columns]
    if missing_required:
        missing_fmt = ", ".join(missing_required)
        raise ValueError(f"Required columns missing from dataset: {missing_fmt}")

    optional_columns = [col for col in extra_columns if col in base_frame.columns]
    columns = list(dict.fromkeys(required_columns + optional_columns))

    frame = base_frame.select(*columns)
