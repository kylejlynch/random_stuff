...
from .rules import RuleSet, ruleset_from_dict, ruleset_from_sql
...
def _coerce_seed_payload(payload) -> list[RuleSet]:
    if isinstance(payload, dict):
        if "sql" in payload:
            return [ruleset_from_sql(str(payload["sql"]))]
        if "clauses" in payload:
            return [ruleset_from_dict(payload)]
        raise ValueError("Seed dictionary must contain either 'sql' or 'clauses'.")
    if isinstance(payload, list):
        seeds: list[RuleSet] = []
        for entry in payload:
            seeds.extend(_coerce_seed_payload(entry))
        return seeds
...
def load_seed_portfolios(paths: Iterable[Path]) -> list[RuleSet]:
    ...
    for path in paths:
        ...
        try:
            ...
            seeds.append(ruleset_from_sql(raw))
            continue
        seeds.extend(_coerce_seed_payload(payload))
    return seeds
